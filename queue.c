 ...
 * */
inline int dequeue(struct queue *store, char *data, int size){
    if (size <= 0){
        return ERR_QUEUE_INVALID_SIZE;
    }
...
inline int enqueue(struct queue *store, char *data, int size){
    if (size <= 0){
        return ERR_QUEUE_INVALID_SIZE;
    }
 ...
 */
inline int exchange(struct queue *readQueue, struct queue *writeQueue, int size){
    if (size <= 0){
        return ERR_QUEUE_INVALID_SIZE;
    }
...
inline int dequeuec(struct queue *store, char *cptr){
    if (store->base == store->pos){
        return ERR_QUEUE_OUT_OF_MEMORY;;
    }

    store->base = store->base + 1;
    *cptr = store->chars[store->pos];
    return 0;
}

// Enqueue a single byte to store->chars
inline int enqueuec(struct queue *store, char c){
...
inline int foldDown(struct queue *store){
...
inline int fenqueue(FILE *readFile, struct queue *store, int size){
    int read = 0;
    if (store->pos + size > store->cap){
        return ERR_QUEUE_OUT_OF_MEMORY;;
    }
...
inline int fdequeue(FILE *writeFile, struct queue *store, int size){
    int difference = 0;
    int write = 0;
    if (store->pos == 0){
        return ERR_QUEUE_EMPTY;
